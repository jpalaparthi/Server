		...
		if IsAthenticatedUser(r) {
			...
			if err != nil {
				...
				if msg == "" {
					msg2 := models.ValidateService_TypeforUpdate(st)
					if msg2 == "" {
						err := session.UpdateByID("vendor", st.ID, st)
						if err != nil {
							w.WriteHeader(400)
							fmt.Fprintln(w, err.Error())
...
func List(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		result, err := session.ListAll("vendor")
		if err != nil {
			w.WriteHeader(400)
			fmt.Fprintln(w, err.Error())
		} else {
...
func ListByID(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		_id := vars["id"]
		fmt.Println(_id)
		result, err := session.ListByID("vendor", _id)
		if err != nil {
			w.WriteHeader(400)
			fmt.Fprintln(w, err.Error())
		} else {
...
func DeleteByID(w http.ResponseWriter, r *http.Request) {
	if r.Method == "DELETE" {
		vars := mux.Vars(r)
		_id := vars["id"]
		err := session.DeleteByID("vendor", _id)
		if err != nil {
			w.WriteHeader(400)
			fmt.Fprintln(w, err.Error())
		} else {
